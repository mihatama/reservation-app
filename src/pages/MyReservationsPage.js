...
import { Reservation, Staff, Questionnaire } from '../models';
import dayjs from 'dayjs';
import { Container, Paper, TableContainer, Table, TableHead, TableRow, TableCell, TableBody, Typography, Button } from '@mui/material';
...
export default function MyReservationsPage() {
  ...
  useEffect(() => {
    if (!userSub) return;
    const subscription = DataStore.observeQuery(Questionnaire).subscribe(({ items }) => {
      const map = {};
      items.forEach(q => {
        map[q.reservationID] = q;
      });
  ...
  if (!userSub) {
    return (
      <Container maxWidth="md" sx={{ mt: 4 }}>
        <Typography variant="h5" gutterBottom>マイ予約一覧</Typography>
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>問診票</TableCell>
                  <TableCell>キャンセル</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {reservations.sort((a, b) => (a.startTime > b.startTime ? 1 : -1)).map((res) => {
                  ...
                  return (
                    <TableRow key={res.id}>
                      ...
                      <TableCell>
                        {questionnaireMap[res.id] ? (
                          <Typography color="textSecondary">記入済み</Typography>
                        ) : (
                          <Button variant="outlined" component={Link} to={`/questionnaire/${res.id}`}>記入する</Button>
                        )}
                      </TableCell>
                      <TableCell>
                        {canCancel ? (
                          <Button variant="contained" color="error" onClick={() => handleCancelReservation(res.id)}>キャンセル</Button>
                        ) : (
